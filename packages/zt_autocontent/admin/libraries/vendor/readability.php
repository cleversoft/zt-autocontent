...
class Readability {
    ...
    protected function grabArticle($page = null) {
        ...
        for ($pt = 0; $pt < count($nodesToScore); $pt++) {
            ...
            if (preg_match('/<img/i', $nodesToScore[$pt]->innerHTML)) {
                $this->initializeNode($parentNode);
                $candidates[] = $parentNode;
                $innerText = $nodesToScore[$pt]->innerHTML;
                $contentScore = 10000;
            }
...
/* function getArticleTitle($html) {
  ...
  $curTitle = $document->getElementsByTagName('title')->item(0)->textContent;

  return $curTitle;
  }
 */
// returns XHTML
function grabArticleHtml($html, $regArray = array()) {
...
function grabArticle($html, $regArray = array()) {
        ...
        if (!$parentNode->hasAttribute('readability')) {
            if (!count($regArray)) {
                ...
                if ($parentNode->hasAttribute('class') && $parentNode->getAttribute('class') != '') {
                    ...
                    if (preg_match('/' . $regArray['ignoreclass'] . '/', $parentNode->getAttribute('class'))) {

                        $readability->value -= 50;
                    } else if (preg_match('/' . $regArray['getclass'] . '/', $parentNode->getAttribute('class'))) {
                ...
                if ($parentNode->hasAttribute('id') && $parentNode->getAttribute('id') != '') {
                    ...
                    if (preg_match('/(' . $regArray['ignoreid'] . ')/', $parentNode->getAttribute('id'))) {
                        $readability->value -= 50;
                    } else if (preg_match('/' . $regArray['getid'] . '/', $parentNode->getAttribute('id'))) {
                        $readability->value += 25;
                    }
                }
            }
