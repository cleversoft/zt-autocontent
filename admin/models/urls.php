 ...
 */
if (!class_exists('AutoContentModelUrls')) {
     ...
     * FeedList Model
     */
    class AutoContentModelUrls extends JModelList {
        ...
        function splitIntroDesc($ohtml) {
            ...
            $html = mb_convert_encoding($ohtml, 'HTML-ENTITIES', "UTF-8"); //$this->convert_to_utf8($ohtml);
            ...
            foreach ($allElements as $node) {
                if (in_array($node->tagName, $headTag)) {
                    $topDiv = $node;
                    break;
                }
            }

            $topDiv = ($topDiv == NULL) ? $this->doc->createElement('div', 'Readability was unable to parse this page for content.') : $topDiv;

            $desciption['intro'] = $topDiv->ownerDocument->saveXML($topDiv);
            $desciption['full'] = str_replace($desciption['intro'], "", $ohtml);
        ...
        function processData($url, $feed) {
            ...
            $html = $this->get_file($url);
            $html = $this->convert_to_utf8($html);

            $r = new Readability($html, $url);
            $r->debug = false;
            $r->revertForcedParagraphElements = false;
            $r->convertLinksToFootnotes = false;

            $r->regexps = array(
                'unlikelyCandidates' => '/combx|comment|community|disqus|extra|foot|header|menu|remark|rss|shoutbox|sidebar|sponsor|ad-break|agegate|pagination|pager|popup|tweet|twitter/i',
                'okMaybeItsACandidate' => '/and|article|body|column|main|shadow|block|image/i',
                'positive' => '/article|body|hentry|main|page|pagination|post|text|blog|story|block|image/i',
                'negative' => '/combx|comment|com-|contact|foot|footer|footnote|masthead|media|meta|outbrain|promo|related|scroll|shoutbox|sidebar|sponsor|shopping|tags|tool|widget/i',
                'divToPElements' => '/<(blockquote|dl|ol|p|pre|ul)/i',
                'replaceBrs' => '/(<br[^>]*>[ \n\r\t]*){2,}/i',
                'replaceFonts' => '/<(\/?)font[^>]*>/i',
                'normalize' => '/\s{2,}/',
                'killBreaks' => '/(<br\s*\/?>(\s|&nbsp;?)*){1,}/',
                ...
                'getclass' => $feed->getclass,
                'ignoreclass' => $feed->ignore_class,
                'getid' => $feed->get_id,
                'ignoreid' => $feed->ignore_id
            );
            ...
            $text = $this->splitIntroDesc($content);
            $title = $this->title_fix($title);
            $introtext = $this->content_fix($text['intro'], $feed);
            $content = $this->content_fix($text['full'], $feed);
            ...
            $introtext = $this->parse_images($introtext, $url, $feed);
            $content = $this->parse_images($content, $url, $feed);
        ...
        function processK2Data($title, $introtext, $content, $feed) {
            ...
            $post['created'] = ($feed->creation_date != '0000-00-00 00:00:00') ? $feed->creation_date : $datenow->toMySQL();
            $post['publish_up'] = ($feed->start_publishing != '0000-00-00 00:00:00') ? $feed->start_publishing : $datenow->toMySQL();
            $post['language'] = "*";
            $post['featured'] = 0;
